...
def test_create_calculation_exponentiation(base_url: str):
    ...
    response = requests.post(f"{base_url}/calculations", json={"type": "exponentiation", "inputs": [2, 3]}, headers=headers)
...
def test_create_calculation_modulus(base_url: str):
    ...
    token_data = register_and_login(base_url, user_data)
    headers = {"Authorization": f"Bearer {token_data['access_token']}"}
    response = requests.post(f"{base_url}/calculations", json={"type": "modulus", "inputs": [10, 3]}, headers=headers)
    assert response.status_code == 201
    assert response.json()["result"] == 1


def test_create_calculation_sqrt(base_url: str):
    user_data = {
        "first_name": "Calc",
        "last_name": "Root",
        "email": f"calc.sqrt{uuid4()}@example.com",
        "username": f"calc_sqrt_{uuid4()}",
        "password": "SecurePass123!",
        "confirm_password": "SecurePass123!"
    }
    token_data = register_and_login(base_url, user_data)
    headers = {"Authorization": f"Bearer {token_data['access_token']}"}
    response = requests.post(f"{base_url}/calculations", json={"type": "sqrt", "inputs": [81]}, headers=headers)
    assert response.status_code == 201
    assert response.json()["result"] == 9


def test_report_summary_with_filters(base_url: str):
    user_data = {
        "first_name": "Report",
        "last_name": "User",
        "email": f"report.user{uuid4()}@example.com",
        "username": f"report_user_{uuid4()}",
        "password": "SecurePass123!",
        "confirm_password": "SecurePass123!"
    }
    token_data = register_and_login(base_url, user_data)
    ...
    requests.post(f"{base_url}/calculations", json={"type": "addition", "inputs": [2, 3]}, headers=headers)
    requests.post(f"{base_url}/calculations", json={"type": "modulus", "inputs": [10, 3]}, headers=headers)
    requests.post(f"{base_url}/calculations", json={"type": "sqrt", "inputs": [81]}, headers=headers)

    response = requests.get(f"{base_url}/reports/summary?calculation_type=modulus", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total_calculations"] == 1
    assert data["count_by_type"]["modulus"] == 1
